...
from common import GameVariables, sprites, EntityWorm, GameState, point2world, draw_dir_indicator, draw_target, fonts, WHITE
from common.vector import Vector, distus, vectorFromAngle
...
class MissionsGamePlay(GamePlayMode):
    ...
    def assign_missions(self, worm: EntityWorm, oldMission=None):
        ...
        for _ in range(3):
            ...
            self._log += f"{worm.name_str} received mission {newMission.missionType}\n"
    ...
    def evaluate_missions(self, worm: EntityWorm, oldMission=None):
        ...
        currentWormMissionsTypes = [i.missionType for i in self.worm_to_missions[worm]]
    ...
    def assign_one_mission(self, worm, oldMission=None):
        ...
        for mission in self.worm_to_missions[worm]:
            available_missions.remove(mission.missionType)
        
        if oldMission:
            available_missions.remove(oldMission.missionType)
    ...
    def draw(self, win: pygame.Surface):
        ...
        yOffset = 0
        for mission in self.worm_to_missions[currentWorm]:
            surf = mission.surf
