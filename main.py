...
def createWorld():
	...
	Game._game.mapChoice = imageChoice
...
def boom(pos, radius, debries = True, gravity = False, fire = False):
	...
	for p in listToCheck:
		...
		totalRadius = radius * shockRadius
...
def splash(pos, vel):
	for i in range(10 + int(vel.getMag())):
		d = Debrie(Vector(pos.x, mapHeight - Water.level - 3), 10, [Water.waterColor[1]], 1, False, True)
...
class Water:
	...
	def draw(self, offsetY=0):
		...
		for i in range(times):
			...
			y =  int(mapHeight - Water.level - 3 - Water.waterAmp - offsetY) - int(camPos.y)
...
class BackGround:
	...
	def draw(self):
		...
		win.blit(pygame.transform.scale(self.imageSky, (win.get_width(), mapHeight)), (0,0 - camPos[1]))
...
def giveGoodPlace(div = 0, girderPlace = True):
	...
	if Game._game.fortsMode and not div == -1:
		half = mapWidth / teamManager.totalTeams
	...
	while r < 2 * pi:
		...
		if testPos.x >= mapWidth or testPos.y >= mapHeight - Water.level or testPos.x < 0:
			if Game._game.mapClosed:
				...
				continue
		if testPos.y < 0:
			if gameMap.get_at((int(testPos.x), 0)) == GRD:#Game._game.mapClosed and 
				return False
			else:
				r += pi /8
				continue
...
class PhysObj:
	...
	def step(self):
		...
		if Game._game.warpedMode and camTrack == self:
			if ppos.x > mapWidth:
				ppos.x = 0
				camPos.x -= mapWidth
			if ppos.x < 0:
...
class Worm (PhysObj):
	...
	def damage(self, value, damageType=0):
		if self.alive:
			...
			if not self == objectUnderControl:
				if not sentring and not raoning and not waterRising and not self in teamManager.currentTeam.worms:
			...
			if Game._game.gameMode == TERMINATOR and victim == self and not terminatorHit:
	...
	def draw(self):
		...
		if len(self.team.artifacts) > 0 and holdArtifact:
			if self is objectUnderControl and weaponMan.getCategory(weaponMan.currentWeapon) == CATEGORY_ARTIFACTS:
	...
	def dieded(self, cause=-1):
		...
		if Game._game.gameMode == TERMINATOR and self == victim:
			teamManager.currentTeam.points += 1
			addToKillList()
...
class Mine(PhysObj):
	...
	def secondaryStep(self):
		...
		if Game._game.darkness and self.activated:
			lights.append((self.pos[0], self.pos[1], 50, (100,0,0,100)))
	def deathResponse(self):
		boom(self.pos, 30)
	def draw(self):
		if Game._game.diggingMatch:
...
class HealthPack(PetrolCan):
	...
	def secondaryStep(self):
		...
		if distus(objectUnderControl.pos, self.pos) < (self.radius + objectUnderControl.radius + 5) * (self.radius + objectUnderControl.radius + 5)\
			and not objectUnderControl.health <= 0:
			self._reg.remove(self)
...
class SentryGun(PhysObj):
	...
	def __init__(self, pos, teamColor):
		...
		self.surf.blit(sprites, (0,0), (80, 32, 17, 26))
...
class Venus:
	...
	def __init__(self, pos, angle = -1):
		...
		self.surf.blit(sprites, (0,0), (0, 64, 48, 16))
	def step(self):
...
class GreenShell(PhysObj):
	...
	def draw(self):
		if not self.speed == 0:
			...
			index = 0	
		win.blit(sprites, point2world(self.pos - Vector(16,16)/2), ((index*16, 48), (16,16)))
...
class SnailShell(PhysObj):
	def __init__(self, pos, direction, energy):
		...
		self.surf.blit(sprites, (0,0), (64,48, 6,6))
	def collisionRespone(self, ppos):
...
class Snail:
	...
	def __init__(self, pos, anchor, clockwise=RIGHT):
		...
		self.life = 0
		self.surf = pygame.Surface((6,6), pygame.SRCALPHA)
		self.surf.blit(sprites, (0,0), (70,48,6,6))
...
class Bubble:
	...
	def step(self):
		...
		if not self.catch:
			...
			self.catch.vel *= 0
		if Game._game.mapClosed and (self.pos.x - self.radius <= 0 or self.pos.x + self.radius >= mapWidth - Water.level):
			self.burst()
...
class MjolnirReturn:
	...
	def step(self):
		...
		gameDistable()
		if distus(self.pos, objectUnderControl.pos) < objectUnderControl.radius * objectUnderControl.radius * 2:
			Game._game.nonPhys.remove(self)
			global holdArtifact
			holdArtifact = True
	def draw(self):
...
class Mjolnir(PhysObj):
	...
	def secondaryStep(self):
		...
		if dist(objectUnderControl.pos, self.pos) < self.radius + objectUnderControl.radius + 5 and not objectUnderControl.health <= 0\
			...
			PhysObj._reg.remove(self)
			commentator.que.append((objectUnderControl.nameStr, ("", " is worthy to wield mjolnir!"), teamManager.currentTeam.color))
...
class MagicLeaf(PhysObj):
	def __init__(self, pos):
		...
		self.angle = 0
		self.surf = pygame.Surface((16,16), pygame.SRCALPHA)
		self.surf.blit(sprites, (0,0), (48, 64, 16,16))
	def secondaryStep(self):
		if self.vel.getMag() > 0.25:
			self.angle += self.vel.x*4
		if distus(objectUnderControl.pos, self.pos) < (self.radius + objectUnderControl.radius + 5)**2 \
			and not objectUnderControl.health <= 0\
			and not len(objectUnderControl.team.artifacts) > 0:
			PhysObj._reg.remove(self)
			commentator.que.append((objectUnderControl.nameStr, ("", " became master of plants"), teamManager.currentTeam.color))
...
class Icicle(LongBow):
	...
	def stamp(self):
		...
		ground.blit(surf, self.pos - tup2vec(surf.get_size())//2)
		...
		surf = pygame.transform.rotate(self.surf, -degrees(self.vel.getAngle()))
		gameMap.blit(surf, self.pos - tup2vec(surf.get_size())//2)
...
class EarthSpike:
	def __init__(self):
		...
		self.pos = checkPotential(objectUnderControl, 25)[-1]
		Game._game.nonPhys.append(self)
		self.timer = 0
		self.surf = pygame.Surface((32, 32), pygame.SRCALPHA)
		self.surf.blit(sprites, (0,0), ((32, 96), (32, 32)))
		if randint(0, 1) == 0:
			self.surf = pygame.transform.flip(self.surf, True, False)
	...
	def step(self):
		...
		if self.timer == 5:
			...
			stain(self.pos - Vector(0, 3), imageHole, (32,32), True)
		...
		if self.timer == 6:
			...
			ground.blit(self.surf, rectPos)
			surf = self.surf.copy()
			pixels = pygame.PixelArray(surf)
			for i in self.colors:
				pixels.replace(i, GRD)
			del pixels
			gameMap.blit(surf, rectPos)
		self.timer += 1
	def draw(self):
		pass
...
class Tornado:
	def __init__(self):
		self.width = 30
		self.pos = objectUnderControl.pos + Vector(objectUnderControl.radius + self.width / 2, 0) * objectUnderControl.facing
		self.facing = objectUnderControl.facing
		Game._game.nonPhys.append(self)
	...
	def step(self):
		...
		for swirl in self.swirles:
			swirl[2] += 0.1
		rect = (Vector(self.pos.x - self.width / 2, 0), Vector(self.width, mapHeight))
		for obj in PhysObj._reg:
...
class Avatar(PhysObj):
	def __init__(self, pos):
		...
		camTrack = self
		self.surf = pygame.Surface((16,16), pygame.SRCALPHA)
		self.surf.blit(sprites, (0,0), (0,112,16,16))
	def secondaryStep(self):
		self.angle -= self.vel.x*4
		# pick up
		if dist(objectUnderControl.pos, self.pos) < self.radius + objectUnderControl.radius + 5 and not objectUnderControl.health <= 0\
	...
	def removeFromGame(self):
		if self in PhysObj._reg:
			PhysObj._reg.remove(self)
		worldArtifacts.append(AVATAR)
	def draw(self):
		angle = 45 * round(self.angle / 45)
		surf = pygame.transform.rotate(self.surf, angle)
		win.blit(surf , point2world(self.pos - tup2vec(surf.get_size())/2))
	def comment(self):
		commentator.que.append(("", ("who is the next avatar?", ""), HUDColor))
...
class WeaponManager:
	...
	def switchWeapon(self, string):
		...
		if canShoot():
			...
			if string in ["flare", "artillery assist"]:
				blitWeaponSprite(weaponHold, (0,0), "flare")
				return
	...
	def drawWeaponIndicators(self):
		...
		if weaponMan.currentWeapon == "earth spike" and state == PLAYER_CONTROL_1 and teamManager.currentTeam.ammo("earth spike") != 0:
			pot = checkPotential(objectUnderControl, 25)
			drawTarget(pot[-1])
...
def cycleWorms():
	global objectUnderControl, camTrack, run, nextState
	global deploying, sentring, deployPacks, switchingWorms, raoning, waterRising, trigerArtifact
	global deployingArtifact
	...
	if Game._game.damageThisTurn > Game._game.mostDamage[0]:
		Game._game.mostDamage = (Game._game.damageThisTurn, objectUnderControl.nameStr)	
	...
	# deploy pack:
	if deployPacks and Game._game.roundCounter % teamManager.totalTeams == 0 and not deploying:
		deploying = True
		Game._game.roundCounter -= 1
		nextState = WAIT_STABLE
	...
	if Game._game.artifactsMode:
		...
		if len(worldArtifacts) > 0 and not deployingArtifact:
			chance = randint(0,10)
			if chance == 0 or trigerArtifact:
				trigerArtifact = False
				deployingArtifact = True
...
def switchWorms():
	...
	currentWorm = teamManager.currentTeam.worms.index(objectUnderControl)
	totalWorms = len(teamManager.currentTeam.worms)
	currentWorm = (currentWorm + 1) % totalWorms
	objectUnderControl = teamManager.currentTeam.worms[currentWorm]
	camTrack = objectUnderControl
...
def stateMachine():
	...
	global objectUnderControl, camTrack, gameStableCounter, shotCount, fireWeapon, run, allowAirStrikes
	if state == RESET:
		gameStable = False
		playerControl = False
...
if __name__ == "__main__":
	...
	while run:
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1: # left click (main)
				...
				if playerControlPlacing:
					teamManager.teams[teamManager.teamChoser].addWorm((mousePos[0]/scalingFactor + camPos.x, mousePos[1]/scalingFactor + camPos.y))
					teamManager.teamChoser = (teamManager.teamChoser + 1) % teamManager.totalTeams
		...
		mousePos = pygame.mouse.get_pos()
		if playerScrollAble and pygame.mouse.get_focused() and RadialMenu.menu is None:
			scroll = Vector()
		...
		for p in PhysObj._reg:
			...
			if not p.stable:
				gameDistable()
		for f in Game._game.nonPhys: f.step()
		...
		# draw health bar
		if not state in [RESET, GENERATE_TERRAIN, PLACING_WORMS, CHOOSE_STARTER] and drawHealthBar: HealthBar._healthBar.step()
		if not state in [RESET, GENERATE_TERRAIN, PLACING_WORMS, CHOOSE_STARTER] and drawHealthBar: HealthBar._healthBar.draw() # teamHealthDraw()
		if Game._game.gameMode == TERMINATOR and victim and victim.alive:
			drawDirInd(victim.pos)
		if Game._game.gameMode == TARGETS and objectUnderControl:
			for target in ShootingTarget._reg:
		...
		# debug:
		if damageText[0] != Game._game.damageThisTurn: damageText = (Game._game.damageThisTurn, pixelFont5.render(str(int(Game._game.damageThisTurn)), False, HUDColor))
		...
		# draw loading screen
		if state in [RESET, GENERATE_TERRAIN] or (state in [PLACING_WORMS, CHOOSE_STARTER] and not Game._game.manualPlace):
			win.fill((0,0,0))
			pos = (winWidth/2 - loadingSurf.get_width()/2, winHeight/2 - loadingSurf.get_height()/2)
